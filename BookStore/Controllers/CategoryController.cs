...
namespace BookStore.Controllers
{
    public class CategoryController : Controller
    {
        ...
        // GET: /<controller>/
        public IActionResult Detail(int id,int page =1,int pagesize = 2)
        {

            //List<Book> books = _bookService.GetAllBookOfCategory(id);
            int totalRecord = 0; //tổng tất cả record trong category sách
            List<Book> books = _bookService.GetAllBookOfCategory(id,ref totalRecord,page,pagesize);
            ViewBag.Total = totalRecord;//
            ViewBag.Page = page;
            int maxPage = 5;
            int totalPage = 0;
            totalPage = (int)Math.Ceiling((double)totalRecord / pagesize);//Làm tròn 
            ...
            ViewData["category"] = _bookService.GetAllCategoryCount(); // Gữi category
